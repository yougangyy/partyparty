$(function () {
    ...
    // var nDivHight = $(".notLanded").height();
    // var son = $(".son").height();
    // $(".notLanded").scroll(function () {
    //     nScrollHight = $(this)[0].scrollHeight;
    //     nScrollTop = $(this)[0].scrollTop;
    //     if (nScrollTop + nDivHight >= nScrollHight - son * 2) {
    //         $("#login").attr("class") == "active"
    //         arrData.push({
    //             img: "./image/invitation-header.png",
    //             span: "Edsenni",
    //             i: "暂未登陆"
    //         }, {
    //             img: "./image/invitation-header.png",
    ...
    if (/iphone|ipad|ipod|ios/.test(ua)) { //ios操作
        ...
        setupWebViewJavascriptBridge(function (bridge) {
            function getUid(data) {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://api.ppparty.cn/h5user/getInviteDetail",
                    data: data_,
                    dataType: "text",
                ...
                $("#button").on("click", function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "http://api.ppparty.cn/h5user/withdrawCoin",
                        data: datas_,
                        dataType: "text",
                ...
                $(".footButton").on("click", function () {
                    bridge.callHandler('callObjectiveCNativeShareMethod', {
                            // 分享 URL
                            "shareURL": "http://h5.ppparty.cn/register/index.html?uid=" + uids + "&time=" + tiemrs,
        ...
        $(".footButton").on("click", function () {
            PartyAndroidJSHandler.showShare("Hi~快来加入PartyParty和我一起互动吧！", "http://h5.ppparty.cn/register/index.html?uid=" + uids + "&time=" + tiemrs); //调取安卓分享接口
        })
    ...
    // new ClipboardJS('#buttonCodde', {
    //     text: function(trigger) {
    //      return trigger.getAttribute('textCode'); // 返回需要复制的内容
    //     }
    //    });
