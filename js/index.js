...
$(function(){
    ...
    // var newurl = updateQueryStringParameter(window.location.href, 'uid', '4LIERaPe9I/cDx5o2N7vvA==');
    // //向当前url添加参数，没有历史记录
    ...
    $(".tel-val").on("blur", function () {
            if (!(/^1(3|4|5|6|7|8|9)\d{9}$/.test($(".tel-val").val()))) {
                ...
                $(".errorImg").attr('src', "http://h5.ppparty.cn/register/image/warning.png");
    ...
    $(".obtain").on("touchend", function () {
        ...
        if (show === true) {
            ...
            $.ajax({
                type: "POST",
                url: "http://api.ppparty.cn/h5user/sendPhone",
                data: encryptData,
                cache: false,
                async: false,
                dataType: "text",
                success: function (data) {
                    ...
                    if(newData.code == 0){
                        $(".errorImg").attr('src', "http://h5.ppparty.cn/register/image/success-index.png");
                        // 关闭开关，让用户无法点击
                        show = false;
                        ...
                        $(".errorImg").attr('src', "http://h5.ppparty.cn/register/image/error-index.png");
                    }
                    $(".error").css("display", "block")
                    styleCss()
                    setTimeout(function () {
    ...
    $('.register-button').on('touchend',function(){
        ...
        var uid = JSON.parse(decrypt(getUrlParam("uid")))
        // 600002
        var _data={
            account:tel,
            type:0,
        ...
        $.ajax({
            type: "POST",
            url: "http://api.ppparty.cn/h5user/userRegister",
            data: after,
            dataType: "text",
            success: function (response) {
             ...
             if($(".verification-val").val() == ""){
                ...
                if(data.code == 0){
                    $(".errorImg").attr('src', "http://h5.ppparty.cn/register/image/success-index.png");
                window.location.href="http://h5.ppparty.cn/successful/successful.html" 
...
$(".clicks").on("touchend",function(){
    window.location.href="http://h5.ppparty.cn/download/download.html"
})
